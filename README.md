# Excel-Bike-Sales-Dashboard
# Bike Sales Dashboard — Excel

##  Overview
An interactive Excel dashboard analyzing bike sales data — exploring how customer demographics like age, gender, and commute distance relate to bike purchase decisions.

##  Objective
To practice the full Excel analyst workflow: cleaning raw data, building Pivot Tables, creating calculated fields, and designing an interactive dashboard with slicers.

##  Tools & Skills Used
- **Pivot Tables** — summarizing purchase data by multiple dimensions
- **Calculated Columns** — nested IF formulas to bucket customers into age brackets
- **Slicers** — interactive filtering across the dashboard
- **Charts** — column and line charts to visualize trends
- **Data Cleaning** — fixed inconsistent labels and removed incorrect Grand Total values from chart data

##  What the Dashboard Shows
- **Average income** of customers who purchased a bike vs. those who didn't, broken down by gender
- **Customer age bracket** vs. bike purchase decision
- **Customer commute distance** vs. bike purchase decision

##  Key Insights
- Customers with higher average income were more likely to purchase a bike across both genders
- Middle-aged customers showed the highest purchase activity compared to adolescents and older age groups
- Customers commuting longer distances (10+ miles) were more likely to purchase a bike — likely cycling as a transport alternative

##  Files in This Repository
- `Bike_Sales_Project.xlsx` — full Excel workbook (raw data, working sheet, pivot tables, dashboard)
- `dashboard_screenshot.png` — preview image of the final dashboard

##  How to View
Download the `.xlsx` file and open in Microsoft Excel (2010 or later, for slicer support) to interact with the dashboard yourself.


Part of my Data Analyst portfolio.
